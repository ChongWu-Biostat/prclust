...
// [[Rcpp::export]]
List PRclust3Original(NumericMatrix data, NumericMatrix mu3, NumericMatrix theta3,double lambda1, double lambda2, double tau, int methods = 0)
{
    int count = 0;
    int ndim = data.nrow();
    int numbers = data.ncol();
...
// lambda2: the tuning parameter for theta
// [[Rcpp::export]]
List DCADMM(NumericMatrix data, double rho, double lambda2, double tau ,double abs_res = 0.5, double rel_res = 0.5)
{
    int count = 0;
    int ndim = data.nrow();
    int numbers = data.ncol();
    ...
    while ((S2 < S1 || count < 2 ) && count < 500 && stopping_criteria(mu,mu2, ndim, numbers, count,epsilon)==0) {
        ...
        //stopping_criteria2(mu, theta, theta1,u, lambda1, abs_res, rel_res, ndim, numbers, tempcount, methods)==0
...
// lambda2: the tuning parameter for theta
// [[Rcpp::export]]
List DCADMMWarm(NumericMatrix data,NumericMatrix mus,NumericMatrix thetas, double rho, double lambda2, double tau ,double abs_res = 0.5, double rel_res = 0.5)
{
    int count = 0;
    int ndim = data.nrow();
    int numbers = data.ncol();
    ...
    for(int i = 0;i<ndim;i++){
        for (int j =0; j<theta_num; j++) {
            theta(i,j) = thetas(i,j);
            theta2(i,j) = thetas(i,j);
        }
    }
    ...
    //stopping_criteria(mu,mu1, ndim, numbers, count,epsilon)==0
    while ((S2 < S1 || count < 2) && count < 1500) {
        int tempcount = 0;
        //stopping_criteria2(mu, theta, theta1,u, lambda1, abs_res, rel_res, ndim, numbers, tempcount, methods)==0
        while (stopping_criteria2(mu, theta, theta1,u, lambda1, abs_res, rel_res, ndim, numbers, tempcount)==0) {
            for (int j=0; j<numbers; j++) {
